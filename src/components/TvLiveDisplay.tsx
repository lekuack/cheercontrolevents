          ...
          >
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 bg-[#0f172a] z-10 shadow-md">
                <tr className="border-b-2 border-white/20 text-purple-300 text-sm 2xl:text-2xl 3xl:text-3xl font-black uppercase tracking-wider">
                  <th className="py-4 px-4 2xl:py-6 2xl:px-8">#</th>
                  <th className="py-4 px-4 2xl:py-6 2xl:px-8">Equipo / Club</th>
                  <th className="py-4 px-4 2xl:py-6 2xl:px-8 text-center">Hora Presentación</th>
                  <th className="py-4 px-4 2xl:py-6 2xl:px-8 text-center">Estado</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/15">
                {tickerSchedules.map((s) => (
                  <tr key={s.id} className={`hover:bg-white/5 transition-colors ${s.status === "COMPETING" ? "bg-red-500/20 font-bold" : ""}`}>
                    <td className="py-4 px-4 2xl:py-7 2xl:px-8 font-mono font-black text-amber-400 text-lg 2xl:text-3xl 3xl:text-4xl">#{s.orderIndex}</td>
                    <td className="py-4 px-4 2xl:py-7 2xl:px-8">
                      <div className="flex items-center gap-4 2xl:gap-7 min-w-0">
                        {s.team?.institution.logoUrl ? (
                          <img
                            src={s.team.institution.logoUrl}
                            alt={s.team.institution.name}
                            className="w-11 h-11 2xl:w-20 2xl:h-20 3xl:w-24 3xl:h-24 rounded-2xl object-cover border-2 border-white/40 shrink-0 bg-black/40 shadow-md"
                          />
                        ) : (
                          <div className="w-11 h-11 2xl:w-20 2xl:h-20 3xl:w-24 3xl:h-24 rounded-2xl bg-gradient-to-tr from-primary to-purple-600 border-2 border-white/40 flex items-center justify-center font-black text-white text-sm 2xl:text-3xl shrink-0 shadow-md">
                            {s.team?.name?.charAt(0) || "🏆"}
                          </div>
                        )}
                        <div className="flex flex-wrap items-baseline gap-2 2xl:gap-4 min-w-0">
                          <span className="font-black text-white text-lg 2xl:text-3xl 3xl:text-4xl tracking-tight">{s.team?.name}</span>
                          <span className="text-slate-200 font-bold text-sm 2xl:text-2xl 3xl:text-3xl truncate">({s.team?.institution.name})</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-4 2xl:py-7 2xl:px-8 text-center font-mono font-black text-cyan-300 text-lg 2xl:text-3xl 3xl:text-4xl">{formatTime(s.scheduledPerformance)}</td>
                    <td className="py-4 px-4 2xl:py-7 2xl:px-8 text-center whitespace-nowrap">
                      <span className={`px-4 py-2 2xl:px-6 2xl:py-3 rounded-2xl text-xs 2xl:text-2xl font-black tracking-wide inline-flex items-center gap-2 ${s.status === "COMPETING"
